...
const PANELS = [
  {
    id: "investigations",
    label: "Projecten",
    description: "Dossiers, beelden & updates",
    icon: LayoutGrid,
  },
  {
    id: "page",
    label: "Overzichtspagina",
    description: "Opening, status & afsluiting",
    icon: Globe2,
  },
  {
    id: "process",
    label: "Verhaal & werkwijze",
    description: "Achtergrond, proces & inzenden",
    icon: BookOpen,
  },
  {
    id: "updates",
    label: "Alle updates",
    description: "Doorzoek alle onderzoeken",
    icon: FileText,
  },
  {
    id: "publish",
    label: "Publicatie",
    description: "Toegang, controle & versies",
    icon: ShieldCheck,
  },
];
...
function EditorNav({ items, value, onChange, label }) {
  return (
    <nav className="rp-work-tabs" aria-label={label}>
      {items.map((item) => (
        <button
          key={item.id}
          type="button"
...
export default function RembrandtProjectManager({
      ...
      .then(
        ({
          ...
          setSelectedInvestigationId(
            normalized.investigations?.[0]?.id || null,
          );
  ...
  const createPreviewLink = async () => {
    setPreviewBusy(true);
    try {
      const result = await createRembrandtPreviewLinkAsync({
        days: previewDays,
      });
  ...
  const copyPreviewLink = async () => {
    try {
      ...
      onShowToast(
        "Kopiëren is mislukt. Selecteer en kopieer de link handmatig.",
        "error",
      );
    }
  };
  ...
  const selectedInvestigation =
    project.investigations.find(
      (entry) => entry.id === selectedInvestigationId,
    ) || null;
  ...
  const publicationIssues = useMemo(() => {
    ...
    for (const investigation of project.investigations) {
      if (
        ...
        dossierSlugs.has(investigation.slug)
      ) {
        issues.push(
          `${investigation.title?.nl || "Dit dossier"} heeft een unieke URL-slug nodig; ‘preview’ is gereserveerd.`,
        );
      }
  ...
  const overviewStats = [
    ...
    },
    {
      ...
      detail: publicationIssues.length
        ? "aandachtspunten"
        : "klaar voor controle",
  ...
  const save = async ({ publish = false } = {}) => {
    ...
    try {
      ...
      let result;
      try {
        result = await saveRembrandtProjectDataAsync(
          project,
  ...
  const changePublicAccess = async (enabled) => {
    ...
    if (enabled && dirty) {
      onShowToast(
        "Sla uw inhoudswijzigingen eerst op voordat u de publieke toegang verandert.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      let result;
      try {
        result = await setRembrandtProjectAccessAsync(
          enabled,
          currentExpectedVersion,
        );
      } catch (firstError) {
      ...
      setProject((current) =>
        !enabled && dirty ? { ...current, isEnabled: false } : result.project,
      );
      setSavedVersion(result.version);
      setSavedSnapshot(JSON.stringify(result.project));
      onPublished(result.project);
  ...
  const changeUpdateInvestigationFilter = (investigationId) => {
    ...
    const nextUpdate = [...project.updates]
      .filter(
        (update) =>
          investigationId === "all" ||
          update.investigationId === investigationId,
      )
  ...
  const restoreRevision = async (revisionId) => {
    ...
    try {
      ...
      const restored = normalizeRembrandtProject({
        ...revision.content,
        isEnabled: savedPublicEnabled,
      });
      setProject(restored);
  ...
  const removeOrderedEntry = (collection, id) => {
    if (
      ...
      onShowToast(
        "Verplaats of verwijder eerst de gekoppelde updates.",
        "error",
      );
      return;
    }
    const nextSelection =
      collection === "investigations"
        ? project.investigations
            ...
            .sort((a, b) => Number(a.sortOrder) - Number(b.sortOrder))[0]?.id ||
          null
        : null;
    setProject((current) => ({
      ...current,
    ...
    <div className="rp-admin rp-workspace">
      <header className="rp-admin-header">
        <div>
          ...
          <span>De werkruimte voor uw onderzoeken.</span>
        </div>
      ...
      {needsRepair && (
        <aside className="rp-admin-data-alert" role="alert">
          ...
          <div>
            ...
            <p>
              Er waren ongeldige of dubbele dossier-sleutels. De werken zijn
              tijdelijk opnieuw uniek gemaakt; sla de wijzigingen op om dit ook
              definitief te bewaren.
            </p>
            <ul>
      ...
      >
        {PANELS.map((entry) => (
          <button
            ...
            onClick={() => {
              setPanel(entry.id);
              if (entry.id === "investigations") setWorkspaceMode("overview");
              if (entry.id === "updates")
                changeUpdateInvestigationFilter("all");
            }}
          >
            ...
            {entry.id === "investigations" && (
              <span>{project.investigations.length}</span>
            )}
            {entry.id === "process" && (
              <span>{project.researchSteps.length}</span>
            )}
          </button>
        ))}
      </div>
      <div className="rp-work-main">
        <div className="rp-work-context">
          <div>
            <span>
              WERKRUIMTE /{" "}
              {PANELS.find((entry) => entry.id === panel)?.label.toUpperCase()}
            </span>
            <h2>
              {panel === "investigations" && workspaceMode === "overview"
                ? "Uw onderzoeken"
                : PANELS.find((entry) => entry.id === panel)?.label}
            </h2>
          </div>
          <div>
            <span className="rp-work-language-label">Inhoud bewerken in</span>
            <LanguageTabs language={language} onChange={setLanguage} />
          </div>
        </div>
        {panel === "investigations" && workspaceMode === "overview" && (
          <div
            className="rp-admin-overview-strip"
            aria-label="Projectoverzicht"
          >
            {overviewStats.map((stat) => (
              <div
                key={stat.label}
                className={stat.tone ? `is-${stat.tone}` : ""}
              >
        ...
        {panel === "investigations" && workspaceMode === "overview" && (
          <section
            className="rp-project-overview"
            id="rp-panel-investigations"
            role="tabpanel"
            aria-labelledby="rp-tab-investigations"
          >
            <div className="rp-work-intro">
              <p>
                Elk onderzoek heeft een eigen pagina. Kies een project om de
                inhoud, beelden en onderzoekstijdlijn te beheren.
              </p>
              <button
                type="button"
                className="admin-button admin-button--primary"
            ...
            <div className="rp-project-grid">
              {[...project.investigations]
                .sort((a, b) => a.sortOrder - b.sortOrder)
                .map((entry) => (
                  <button
                    type="button"
                    className="rp-project-card"
                    key={entry.id}
                    onClick={() => {
                      setSelectedInvestigationId(entry.id);
                      setWorkspaceMode("edit");
                      setDossierTab("text");
                    }}
                  >
                    <div className="rp-project-card__art">
                      {entry.coverImage ? (
                    ...
                    <div className="rp-project-card__body">
                      <span>{entry.title?.nl || "Nieuw project"}</span>
                      <h3>
                        {entry.subtitle?.nl || "Naam van het werk toevoegen"}
                      </h3>
                      <p>
                        {entry.visible === false
                          ? "Verborgen"
                          : savedPublicEnabled
                            ? "Publiek zichtbaar"
                            : "Klaargezet · website verborgen"}
                      </p>
                      <div>
                        <span>
                          {
                            project.updates.filter(
                              (update) => update.investigationId === entry.id,
                            ).length
                          }{" "}
                          updates · {(entry.gallery || []).length}{" "}
            ...
            <div className="rp-overview-caption">
              <Globe2 />
              <div>
                <strong>De introductie boven de projectkaarten</strong>
                <p>
                  Deze tekst verschijnt op de overzichtspagina van Lost
                  Rembrandt.
                </p>
              </div>
            </div>
            <div className="rp-admin-form-grid rp-admin-form-grid--two">
              <Field label="Sectietitel">
                <input
                  ...
                  onChange={(event) =>
                    updateSettings("investigationsTitle", event.target.value)
                  }
                />
              </Field>
              <Field label="Inleiding">
                <textarea
                  ...
                  onChange={(event) =>
                    updateSettings("investigationsIntro", event.target.value)
                  }
                />
              </Field>
            </div>
        ...
        {panel === "page" && (
          <EditorNav
            label="Onderdelen overzichtspagina"
            value={pageTab}
            onChange={setPageTab}
            items={[
              { id: "0", label: "Introductie" },
              { id: "1", label: "Status" },
              { id: "2", label: "Hoofdbeeld" },
              { id: "3", label: "Onderzoeksfases" },
              { id: "4", label: "Afsluiting" },
            ]}
          />
        )}
        {panel === "process" && (
          <EditorNav
            label="Verhaal en werkwijze"
            value={processTab}
            onChange={setProcessTab}
            items={[
              { id: "0", label: "Over het initiatief" },
              { id: "1", label: "Onderzoeksproces" },
              { id: "2", label: "Een werk inzenden" },
            ]}
          />
        )}
        {panel === "publish" && (
          <EditorNav
            label="Publicatieonderdelen"
            value={publishTab}
            onChange={setPublishTab}
            items={[
              { id: "1", label: "Toegang & controle" },
              { id: "0", label: "Zoekmachines & delen" },
              { id: "2", label: "Versiegeschiedenis" },
              { id: "3", label: "Herstellen" },
            ]}
          />
        )}
        {panel === "page" && (
          ...
          >
                ...
                .map((investigation) => (
                  ...
                  >
                    {investigation.coverImage ? (
                      <img
                        className="rp-case-thumb"
                        src={investigation.coverImage}
                        alt=""
                      />
                    ) : (
                      <span>
                        {String(investigation.sortOrder).padStart(2, "0")}
                      </span>
                    )}
                    <div>
                      <strong>
                        {investigation.title?.nl || investigation.id}
                      </strong>
                      <small>
                        {investigation.subtitle?.nl ||
            ...
            {selectedInvestigation ? (
              <section className="rp-admin-editor">
                <header className="rp-admin-editor__header">
                  <div>
                    <span className="rp-admin-status rp-admin-status--draft">
                      Werk{" "}
                      {String(selectedInvestigation.sortOrder).padStart(2, "0")}
                    </span>
                    <h2>
                      {selectedInvestigation.title?.nl || "Nieuw dossier"}
                    </h2>
                    {savedPublicEnabled &&
                      selectedInvestigation.visible !== false && (
                        <a
                          className="rp-live-link"
                          href={localizePath(
                            `${REMBRANDT_PROJECT_ROUTE}/${selectedInvestigation.slug}`,
                            language,
                          )}
                          target="_blank"
                          rel="noopener noreferrer"
                          title="Bekijk de laatst opgeslagen pagina"
                        >
                          <Eye />
                          Bekijk pagina
                        </a>
                    ...
                    <p className="rp-admin-editor__context">
                      {selectedInvestigation.reference || "Zonder referentie"} ·{" "}
                      {selectedInvestigation.slug || "zonder URL-slug"}
                    </p>
                  </div>
                  <div>
                    <button
                      type="button"
                      aria-label="Dossier omhoog"
                      onClick={() =>
                        moveOrderedEntry(
                          "investigations",
                ...
                <EditorNav
                  label="Dossieronderdelen"
                  value={dossierTab}
                  onChange={(tab) => {
                    if (tab === "updates") {
                      changeUpdateInvestigationFilter(selectedInvestigation.id);
                      setPanel("updates");
                    } else setDossierTab(tab);
                  }}
                  items={[
                    { id: "text", label: "Tekst & verhaal" },
                    { id: "media", label: "Beelden" },
                    {
                      id: "updates",
                      label: `Updates (${project.updates.filter((entry) => entry.investigationId === selectedInvestigation.id).length})`,
                    },
                    { id: "settings", label: "Instellingen" },
                  ]}
                />
                <div
                  hidden={dossierTab !== "settings"}
                  className="rp-admin-editor__publication"
                >
                  <div className="rp-admin-editor__publication-fields">
                    <Field label="Referentie">
                      <input
                        value={selectedInvestigation.reference || ""}
                        onChange={(event) =>
                          updateInvestigation(
                            selectedInvestigation.id,
                            "reference",
                    ...
                    <Field label="URL-slug">
                      <input
                        value={selectedInvestigation.slug || ""}
                        onChange={(event) =>
                          updateInvestigation(
                            selectedInvestigation.id,
                            "slug",
                    ...
                    <Field label="Onderzoeksstatus">
                      ...
                      >
                        <option value="discovery">Ontdekking</option>
                        ...
                        <option value="expert-review">Expertbeoordeling</option>
                  ...
                  <div
                    className="rp-admin-editor__publication-toggles"
                    role="group"
                    aria-label="Dossierweergave"
                  >
                    <SwitchControl
                ...
                <div
                  hidden={dossierTab === "settings"}
                  className="rp-admin-editor__body"
                >
                  <div
                    hidden={dossierTab !== "text"}
                    className="rp-admin-form-grid rp-admin-form-grid--two"
                  >
                    ...
                    <Field label="Publieke statusregel">
                      <input
                        value={valueFor(
                          selectedInvestigation,
                          "statusLabel",
                    ...
                    <Field label="Dossierbeschrijving">
                      <textarea
                        rows="7"
                        value={valueFor(
                          selectedInvestigation,
                    ...
                    <div className="rp-admin-media-row">
                      <div className="rp-admin-media-preview">
                        {selectedInvestigation.coverImage ? (
                          <img
                            src={selectedInvestigation.coverImage}
                            alt="Dossier preview"
                          />
                        ) : (
                          <ImageIcon aria-hidden="true" />
                              ...
                              <button
                                ...
                                onClick={() =>
                                  updateInvestigation(
                                    selectedInvestigation.id,
                                    "gallery",
                                    selectedInvestigation.gallery.filter(
        ...
        {panel === "process" && (
          <div
            ...
            id="rp-panel-process"
            role="tabpanel"
            aria-labelledby="rp-tab-process"
          >
            <section hidden={processTab !== "0"} className="rp-admin-card">
                ...
                <button
                  ...
                  onClick={() => {
                    setContentSelection((current) => ({
                      ...current,
                      about: project.aboutSections.length,
                    }));
                    const sortOrder =
                      Math.max(
                        0,
                        ...project.aboutSections.map(
                    ...
                    setProject((current) => ({
                      ...current,
                      aboutSections: [
                        ...current.aboutSections,
                        {
                          id: `about-${sortOrder}`,
                          sortOrder,
                          visible: true,
                          title: { nl: "Nieuwe sectie", en: "", fr: "" },
                          body: { nl: "", en: "", fr: "" },
                        },
                      ],
                    }));
              ...
              <div className="rp-admin-form-grid">
                <Field label="Sectietitel">
                  <input
                    value={valueFor(project.settings, "aboutTitle", language)}
                    onChange={(event) =>
                      updateSettings("aboutTitle", event.target.value)
                    }
                  />
                </Field>
                <Field label="Inleiding">
                  <textarea
                    rows="4"
                    value={valueFor(project.settings, "aboutIntro", language)}
                    onChange={(event) =>
                      updateSettings("aboutIntro", event.target.value)
                    }
                  />
                </Field>
              </div>
              <div className="rp-admin-content-list">
                <div
                  className="rp-content-picker"
                  aria-label="Kies een onderdeel"
                >
                  {[...project.aboutSections]
                    .sort((a, b) => a.sortOrder - b.sortOrder)
                    .map((entry, index) => (
                      <button
                        type="button"
                        ...
                        className={
                          Math.min(
                            ...
                            project.aboutSections.length - 1,
                          ) === index
                            ? "is-active"
                            : ""
                        }
                ...
                {[...project.aboutSections]
                  ...
                  .map((section) => (
                    <article key={section.id}>
                      <div className="rp-admin-content-list__actions">
                        <button
                          type="button"
                          aria-label="Sectie omhoog"
                          onClick={() =>
                            moveOrderedEntry("aboutSections", section.id, -1)
                          }
                        >
                          <ArrowUp aria-hidden="true" />
                        </button>
                        ...
                        <button
                          type="button"
                          aria-label="Sectie tonen of verbergen"
                          onClick={() =>
                            updateAboutSection(
                              section.id,
                              "visible",
                        ...
                        <button
                          ...
                          onClick={() =>
                            removeOrderedEntry("aboutSections", section.id)
                          }
                        >
                          <Trash2 aria-hidden="true" />
                        </button>
                      </div>
                      <Field label={`Titel ${section.sortOrder}`}>
                        <input
                          value={valueFor(section, "title", language)}
                          onChange={(event) =>
                            updateAboutSection(
                              section.id,
                              "title",
                      ...
                      <Field label="Tekst">
                        <textarea
                          rows="6"
                          value={valueFor(section, "body", language)}
                          onChange={(event) =>
                            updateAboutSection(
                              section.id,
                              "body",
            ...
            <section hidden={processTab !== "1"} className="rp-admin-card">
              <div className="rp-admin-card__heading">
                ...
                <button
                  ...
                  onClick={() => {
                    setContentSelection((current) => ({
                      ...current,
                      steps: project.researchSteps.length,
                    }));
                    setProject((current) => ({
                      ...current,
                      researchSteps: [
                        ...current.researchSteps,
                        createResearchStep(current),
                      ],
                    }));
                  }}
                >
              ...
              <div className="rp-admin-form-grid">
                <Field label="Sectietitel">
                  <input
                    value={valueFor(project.settings, "processTitle", language)}
                    onChange={(event) =>
                      updateSettings("processTitle", event.target.value)
                    }
                  />
                </Field>
                <Field label="Inleiding">
                  <textarea
                    rows="4"
                    value={valueFor(project.settings, "processIntro", language)}
                    onChange={(event) =>
                      updateSettings("processIntro", event.target.value)
                    }
                  />
                </Field>
              </div>
              <div className="rp-admin-media-row">
                <div className="rp-admin-media-preview">
                  {project.settings.researchImage ? (
                    <img
                      src={project.settings.researchImage}
                      alt="Preview onderzoeksproces"
                    />
                  ) : (
                    <ImageIcon aria-hidden="true" />
                  )}
                  ...
                  {[...project.researchSteps]
                    ...
                    .map((entry, index) => (
                      <button
                        ...
                        className={
                          Math.min(
                            contentSelection.steps,
                            project.researchSteps.length - 1,
                          ) === index
                            ? "is-active"
                            : ""
                        }
                    ...
                    <article key={step.id}>
                      <div className="rp-admin-content-list__actions">
                        <button
                          ...
                          onClick={() =>
                            moveOrderedEntry("researchSteps", step.id, -1)
                          }
                        >
                          <ArrowUp aria-hidden="true" />
                        </button>
                        ...
                        <button
                          type="button"
                          aria-label="Stap tonen of verbergen"
                          onClick={() =>
                            updateResearchStep(
                              step.id,
                              "visible",
                        ...
                        <button
                          ...
                          onClick={() =>
                            removeOrderedEntry("researchSteps", step.id)
                          }
                        >
                          <Trash2 aria-hidden="true" />
                        </button>
                      </div>
                      <Field label={`Stap ${step.sortOrder}`}>
                        <input
                          value={valueFor(step, "title", language)}
                          onChange={(event) =>
                            updateResearchStep(
                              step.id,
                              "title",
                      ...
                      <Field label="Beschrijving">
                        <textarea
                          rows="4"
                          value={valueFor(step, "body", language)}
                          onChange={(event) =>
                            updateResearchStep(
                              step.id,
                              "body",
              ...
              <div className="rp-admin-form-grid rp-admin-form-grid--two">
                <Field label="Sectietitel">
                  <input
                    ...
                    onChange={(event) =>
                      updateSettings("submissionTitle", event.target.value)
                    }
                  />
                </Field>
                <Field label="Inleiding">
                  <textarea
                    ...
                    onChange={(event) =>
                      updateSettings("submissionIntro", event.target.value)
                    }
                  />
                </Field>
                <Field label="Checklist (één punt per regel)">
                  <textarea
                    rows="7"
                    value={(
                      project.settings.submissionChecklist?.[language] || []
                    ).join("\n")}
                    onChange={(event) =>
                      updateSettings(
                        "submissionChecklist",
                        {
                          ...(project.settings.submissionChecklist || {}),
                          [language]: event.target.value.split("\n"),
                        },
                        false,
                      )
                    }
                ...
                <Field label="Melding bij upload">
                  <textarea
                    rows="4"
                    value={valueFor(
                      project.settings,
                    ...
                    onChange={(event) =>
                      updateSettings("submissionNotice", event.target.value)
                    }
                  />
                </Field>
                <Field label="Titel vertrouwelijkheid">
                  <input
                    value={valueFor(
                      project.settings,
                    ...
                    onChange={(event) =>
                      updateSettings("confidentialityTitle", event.target.value)
                    }
                  />
                </Field>
                <Field label="Tekst vertrouwelijkheid">
                  <textarea
                    rows="4"
                    value={valueFor(
                      project.settings,
                    ...
                    onChange={(event) =>
                      updateSettings("confidentialityText", event.target.value)
                    }
                  />
                </Field>
              </div>
        ...
        {panel === "updates" && updateInvestigationFilter !== "all" && (
          <div className="rp-scoped-update-header">
            <button
              type="button"
              className="rp-back-link"
              onClick={() => {
                setSelectedInvestigationId(updateInvestigationFilter);
                setPanel("investigations");
                setWorkspaceMode("edit");
              }}
            >
            ...
            <strong>
              {
                project.investigations.find(
                  (entry) => entry.id === updateInvestigationFilter,
                )?.subtitle?.nl
              }
            </strong>
            <span>Onderzoekstijdlijn</span>
          </div>
        )}
        {panel === "updates" && (
          <div
          ...
          >
            <aside className="rp-admin-update-list">
              <div className="rp-admin-list-tools">
                ...
                <label className="rp-admin-list-tools__filter">
                  ...
                  >
                    <option value="all">
                      Alle dossiers ({project.updates.length})
                    </option>
                    {[...project.investigations]
                      .sort((a, b) => Number(a.sortOrder) - Number(b.sortOrder))
                      .map((investigation) => (
                        <option key={investigation.id} value={investigation.id}>
                          {investigation.title?.nl ||
                            investigation.reference ||
                            investigation.id}{" "}
                          (
                          {
                    ...
                    >
                      ...
                      <div>
                        ...
                        <small>
                          {project.investigations.find(
                            (investigation) =>
                              investigation.id === update.investigationId,
            ...
            {selectedUpdate ? (
              <section className="rp-admin-editor">
                <header className="rp-admin-editor__header">
                  <div>
                    ...
                    <p className="rp-admin-editor__context">
                      {project.investigations.find(
                        (investigation) =>
                          investigation.id === selectedUpdate.investigationId,
                      )?.reference || "Zonder dossier"}
                    </p>
                  </div>
                  <div>
                ...
                <EditorNav
                  label="Update bewerken"
                  value={updateTab}
                  onChange={setUpdateTab}
                  items={[
                    { id: "text", label: "Verhaal & bevindingen" },
                    { id: "media", label: "Beelden" },
                    { id: "settings", label: "Status & datum" },
                  ]}
                />
                <div
                  hidden={updateTab !== "settings"}
                  className="rp-admin-editor__publication"
                >
                  <div className="rp-admin-editor__publication-fields">
                    <Field label="Onderzoeksdossier">
                      <select
                        value={selectedUpdate.investigationId || ""}
                        onChange={(event) =>
                          updateSelected("investigationId", event.target.value)
                        }
                      >
                ...
                <div
                  hidden={updateTab !== "text"}
                  className="rp-admin-editor__content"
                >
                  <Field label="Titel">
                    <input
                  ...
                  <Field
                    label="Kernbevindingen"
                    hint="Eén bevinding per regel."
                  >
                    <textarea
                      rows="6"
                      value={(
                        selectedUpdate.keyFindings?.[language] || []
                      ).join("\n")}
                      onChange={(event) =>
                        updateSelected(
                          "keyFindings",
                ...
                <div
                  hidden={updateTab !== "media"}
                  className="rp-admin-editor__media"
                >
                  <div className="rp-admin-card__heading">
                    <div>
                      <p>Beeld</p>
        ...
        {panel === "publish" && (
          ...
          >
            ...
            <section hidden={publishTab !== "1"} className="rp-admin-card">
              ...
              <div
                className={`rp-admin-access ${project.isEnabled ? "is-public" : "is-private"}`}
              >
                <div className="rp-admin-access__main">
                  ...
                  <div>
                    <span className="rp-admin-access__eyebrow">
                      Publieke toegang
                    </span>
                    <strong>
                      {project.isEnabled ? "Openbaar" : "Niet openbaar"}
                    </strong>
                    <p>
                      {project.isEnabled
                  ...
                  <div className="rp-admin-access__actions">
                    <span
                      className={`rp-admin-live-state ${project.isEnabled ? "is-live" : "is-offline"}`}
                    >
                ...
                {confirmPublicDisable && project.isEnabled && (
                  <div className="rp-admin-inline-confirm" role="alert">
                    <p>
                      De projectpagina en navigatielink verdwijnen meteen voor
                      bezoekers. Uw inhoud blijft bewaard.
                    </p>
                    <div>
                      <button
                        type="button"
                        className="admin-button admin-button--secondary"
                        onClick={() => setConfirmPublicDisable(false)}
                      >
                        Annuleren
                      </button>
                      <button
                        ...
                        onClick={() => {
                          setConfirmPublicDisable(false);
                          changePublicAccess(false);
                        }}
                      >
                        Project offline halen
                      </button>
                    </div>
                  </div>
                )}
              ...
              <div className="rp-admin-preview-link">
                <div className="rp-admin-preview-link__heading">
                  ...
                  <div>
                    <span className="rp-admin-access__eyebrow">
                      Privévoorbeeld delen
                    </span>
                    <strong>
                      {previewLink
                        ? "Privélink actief"
                        : "Geen actieve privélink"}
                    </strong>
                    <p>
                      De link toont alleen opgeslagen, gepubliceerde updates en
                      verschijnt nergens op de openbare website.
                    </p>
                  </div>
                </div>
                {previewLink ? (
                      ...
                      <div>
                        ...
                        <dd>
                          {new Intl.DateTimeFormat("nl-BE", {
                            dateStyle: "long",
                            timeStyle: "short",
                          }).format(new Date(previewLink.expiresAt))}
                        </dd>
                      </div>
                    </dl>
                    {previewUrl ? (
                      <div className="rp-admin-preview-link__copy">
                        ...
                        >
                          <Copy aria-hidden="true" /> Link kopiëren
                        </button>
                      </div>
                    ) : (
                      <p className="rp-admin-preview-link__notice">
                        Om veiligheidsredenen wordt de bestaande link niet
                        opnieuw getoond. Maak een nieuwe link als u hem opnieuw
                        wilt delen.
                      </p>
                    )}
                    {confirmRevoke ? (
                      <div className="rp-admin-inline-confirm" role="alert">
                        <p>
                          Nieuwe bezoeken worden meteen geblokkeerd. Een reeds
                          geopende preview sluit uiterlijk binnen 20 seconden.
                        </p>
                        <div>
                          <button
                            type="button"
                  ...
                  <p className="rp-admin-preview-link__notice">
                    Maak het project eerst niet openbaar voordat u een privélink
                    aanmaakt.
                  </p>
                ) : (
                  <div className="rp-admin-preview-link__create">
                    <label>
                      ...
                      <select
                        value={previewDays}
                        onChange={(event) =>
                          setPreviewDays(Number(event.target.value))
                        }
                      >
            ...
            {revisions.length === 0 && publishTab === "2" && (
              <section className="rp-admin-card">
                <h2>Nog geen vorige versies</h2>
                <p>
                  Na het opslaan verschijnen eerdere versies van de inhoud hier.
                </p>
              </section>
            )}
            {revisions.length > 0 && (
            ...
            <section
              hidden={publishTab !== "3"}
              className="rp-admin-card rp-admin-card--reset"
            >
              <div>
              ...
              <button
                ...
                onClick={async () => {
                  if (deleteConfirmId === "reset") {
                    try {
                      const template =
                        await fetchRembrandtProjectTemplateAsync();
                      const reset = {
                        ...template,
                        isEnabled: savedPublicEnabled,
                      };
                      ...
                      setSelectedInvestigationId(
                        reset.investigations?.[0]?.id || null,
                      );
                      setDeleteConfirmId(null);
                    } catch (error) {
                      onShowToast(error.message, "error");
