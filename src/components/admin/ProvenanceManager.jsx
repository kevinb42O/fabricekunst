...
export default function ProvenanceManager({ provenanceData, onSaveProvenance, showToast }) {
  ...
  const handleAssetSelect = (section, id) => {
    ...
    update(current => {
      if (!id) return { ...current, [section]: { ...current[section], assetId: '' } };
      const exists = current.assets.some(a => a.id === id);
      if (exists) return { ...current, [section]: { ...current[section], assetId: id } };
      const med = media.find(m => m.id === id);
      const newAsset = {
        id,
        title: { nl: med?.filename || defaultLabel, en: med?.filename || defaultEnglishLabel, fr: med?.filename || defaultFrenchLabel },
        caption: { nl: '', en: '', fr: '' },
        alt: { nl: med?.filename || defaultLabel, en: med?.filename || defaultEnglishLabel, fr: med?.filename || defaultFrenchLabel },
        credit: { nl: '', en: '', fr: '' },
        objectLabel: { nl: '', en: '', fr: '' },
        category: 'context',
        approved: true,
      ...
      return { ...current, assets: [...current.assets, newAsset], [section]: { ...current[section], assetId: id } };
    });
  };
  ...
  const uploadSectionImage = async (section, event) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `Nieuwe ${isContact ? 'contact' : 'hero'}-afbeelding geüpload en gekoppeld. Klik op Publiceren om live te zetten.` });
    } catch (error) {
      setMessage({ type: 'error', text: error.message });
    } finally {
      setBusy(false);
