...
export default function ProvenanceManager({ provenanceData, showToast }) {
  ...
  const publish = async () => { const nextIssues = provenanceIssues(formData, { publishing: true }); setIssues(nextIssues); if (nextIssues.length) { setActiveTab('publish'); setMessage({ type: 'error', text: 'De publicatiecontrole vond aandachtspunten.' }); return; } setBusy(true); try { const saved = await saveProvenanceDraftAsync(formData, version); setVersion(saved.version); const body = await publishProvenanceAsync(saved.version); setFormData(normalizeProvenance(body.provenanceData)); setDirty(false); setIssues([]); setMessage({ type: 'success', text: 'Nieuwe herkomstversie gepubliceerd.' }); showToast?.('Herkomstpagina gepubliceerd.', 'info'); await load(); } catch (error) { setMessage({ type: 'error', text: error.message }); } finally { setBusy(false); } };
