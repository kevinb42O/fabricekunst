...
const parsePriceForSort = (value) => {
  ...
  const hasDecimalSeparator =
    decimalDigits.length > 0 && decimalDigits.length <= 2;
...
export const getItemTranslationStatus = (item) => {
  ...
  const getRawVal = (fieldName, lang) => {
    const camelField = fieldName.replace(/_([a-z])/g, (_, l) =>
      l.toUpperCase(),
    );
...
const getTranslationTooltip = (translationStatus) => {
  ...
  const missingSummary = Object.values(translationStatus.details)
    ...
    .map(
      (language) =>
        `${language.label}: ${language.missing.length} ${language.missing.length === 1 ? "veld" : "velden"}`,
    )
    .join(" · ");
...
export default function ItemManager({
  ...
  const emptyItem = {
    ...
    featured: false,
    condition:
      "Excellente antiquarische staat. Originele band met goudstempels.",
  ...
  const handleItemSurfaceClick = (event, item) => {
    if (
      event.target.closest(
        'button, a, input, select, textarea, label, [role="menu"]',
      )
    )
      return;
    handleEdit(item);
  ...
  const getFormField = (field) => {
    ...
    const langCap =
      formLang.charAt(0).toUpperCase() + formLang.slice(1).toLowerCase();
  ...
  const toggleFieldNvt = (field, lang = formLang) => {
    ...
    const currentEmpty =
      typeof rawEmpty === "object" &&
      rawEmpty !== null &&
      !Array.isArray(rawEmpty)
        ? { ...rawEmpty }
        : {};
    ...
    return (
      <div className="flex items-center justify-between mb-1.5">
        <label className="block text-xs font-mono font-bold text-[#111111] uppercase tracking-wider">
          ...
          <span className="text-[#B8860B] text-[10px] ml-1 font-mono">
        ...
        <button
          ...
          title={
            isNvt
              ? "Klik om weer in te vullen"
              : "Markeer dit veld als N.v.t. / Bewust leeg"
          }
        >
  ...
  const handleCopyAiPrompt = async () => {
    ...
    const fieldsToTranslate = [
      ...
      {
        key: "provenanceDetails",
        label: "Uitgebreid Provenance Verhaal & Veilinghistorie",
      },
      {
        key: "historicalContext",
        label: "Diepgaande Historische & Kunsthistorische Context",
      },
    ];
    ...
    const collectionGroupLabel = getLocalizedCollectionGroup(
      getCollectionGroupForItem(editingItem),
      "nl",
    );
    ...
    fieldsToTranslate.forEach((f) => {
      ...
      const content =
        isNvt || !val || (typeof val === "string" && val.trim() === "")
          ? "[Niet ingevuld / Bewust leeg]"
          : val.trim();
    ...
    if (success && onShowToast) {
      onShowToast(
        `AI-vertaalprompt met ${sourceLangName} als bron is gekopieerd.`,
        "info",
      );
    }
  };
  ...
  const handleImportAiTranslation = () => {
    ...
    if (!data) {
      if (onShowToast)
        onShowToast(
          "Ongeldige JSON. Controleer het resultaat van de AI.",
          "error",
        );
      return;
    }
  ...
  const handleStatusChange = (item, newStatus) => {
    ...
    onSaveItem(updated);
    if (onShowToast)
      onShowToast(`Status van "${item.title}" gewijzigd naar ${newStatus}`);
  };
  ...
  const handleBatchStatusChange = async (newStatus) => {
    ...
    try {
      await Promise.all(
        selectedItems.map((item) => onSaveItem({ ...item, status: newStatus })),
      );
      if (onShowToast)
        onShowToast(
          `Status van ${selectedItems.length} objecten gewijzigd naar "${newStatus}"`,
        );
      setSelectedItemIds([]);
    } catch (error) {
      console.error("Batch status update failed:", error);
      if (onShowToast)
        onShowToast("Niet alle statussen konden worden opgeslagen.", "error");
    }
  };
  ...
  const handleToggleFeatured = (item) => {
    ...
    if (onShowToast)
      onShowToast(
        updated.featured
          ? `Gemarkeerd als Topstuk!`
          : `Topstuk markering verwijderd.`,
      );
  };
  ...
  const handleSaveForm = async (e) => {
    ...
    if (!editingItem.title || !editingItem.title.trim()) {
      if (onShowToast)
        onShowToast("Kan niet opslaan: vul ten minste een titel in.", "error");
      setEditorTab("specs");
      return;
    }
    ...
    if (isUploading) {
      if (onShowToast)
        onShowToast(
          "Wacht tot alle afbeeldingen volledig zijn geüpload.",
          "error",
        );
      return;
    }
    ...
    if (imageUploadError) {
      if (onShowToast)
        onShowToast(
          "Minstens één afbeelding kon niet worden geüpload. Probeer die afbeelding opnieuw voordat u opslaat.",
          "error",
        );
      return;
    }

    const invalidPublishedSaleIndex = (
      editingItem.comparableSales || []
    ).findIndex((sale) => sale.published && !isComparableSaleComplete(sale));
    if (invalidPublishedSaleIndex !== -1) {
      if (onShowToast)
        onShowToast(
          `Comparable sale ${invalidPublishedSaleIndex + 1} is onvolledig of bevat een ongeldige URL of datum.`,
          "error",
        );
    ...
    if (onShowToast)
      onShowToast(`"${editingItem.title}" wordt opgeslagen...`, "loading");

    const mediaUrls = [
      ...(editingItem.images || []).map((image) => image?.url),
      ...(editingItem.comparableSales || []).map((sale) => sale?.imageUrl),
    ];
    try {
      // Index additions precede the catalog write. If this fails there is no
      // content mutation; if the catalog write later fails, any extra index
      // row merely blocks destructive media actions until the next save.
      await syncUniversalMediaUrlsAsync("catalog", editingItem.id, mediaUrls, {
        stage: true,
      });
    } catch (usageError) {
      console.warn(
        "Catalog media usage staging could not be completed:",
        usageError.message,
      );
      if (onShowToast)
        onShowToast(
          "Opslaan is niet gestart: het beeldgebruik kon niet veilig worden geregistreerd. Probeer opnieuw.",
          "error",
        );
      return;
    }
    ...
    try {
      ...
      if (onShowToast)
        onShowToast(
          "Opslaan mislukt. Uw invoer blijft open zodat niets verloren gaat.",
          "error",
        );
      return;
    }
    if (!result || result.success === false || result.error) {
      if (onShowToast)
        onShowToast(
          `Cloudsynchronisatie mislukt: ${result?.error || "onbekende fout"}`,
          "error",
        );
      return;
    }
    try {
      ...
      console.warn(
        "Catalog media usage index could not be updated:",
        usageError.message,
      );
      if (onShowToast)
        onShowToast(
          "Object opgeslagen, maar het beeldgebruik wordt later opnieuw gesynchroniseerd.",
          "info",
        );
    }
    ...
    for (const file of files) {
      try {
        ...
        if (publicUrl) {
          setEditingItem((prev) => ({
            ...prev,
            images: [...prev.images, { url: publicUrl, caption: file.name }],
          }));
        }
      } catch (err) {
        ...
        if (onShowToast)
          onShowToast(
            err?.message || "De foto kon niet worden geüpload.",
            "error",
          );
      }
    }
  ...
  const addComparableSale = () => {
    setEditingItem((prev) => ({
      ...prev,
      comparableSales: [
        ...(prev.comparableSales || []),
        createComparableSale(),
      ],
    }));
  };
  ...
  const removeComparableSale = (index) => {
    ...
    setEditingItem((prev) => ({
      ...prev,
      comparableSales: (prev.comparableSales || []).filter(
  ...
  const moveComparableSale = (index, direction) => {
    setEditingItem((prev) => {
      ...
      [comparableSales[index], comparableSales[targetIndex]] = [
        comparableSales[targetIndex],
        comparableSales[index],
      ];
  ...
  const handleAddImageUrl = (e) => {
    ...
    if (!isR2CatalogImageUrl(newImageUrl.trim())) {
      if (onShowToast)
        onShowToast(
          "Gebruik een afbeelding uit de online mediabibliotheek (media.atelierrembrandt.com).",
          "error",
        );
      return;
    }
    setEditingItem((prev) => ({
      ...
      images: [
        ...(prev.images || []),
        {
          ...
          caption: newImageCaption.trim() || "Afbeelding",
        },
      ],
    }));
    setNewImageUrl("");
  ...
  const filtered = items.filter((item) => {
    ...
    const matchesCategory =
      categoryFilter === "Alle" ||
      getCategorySlug(item.category) === categoryFilter;
  ...
  const sortedItems = [...filtered].sort((first, second) => {
    ...
    if (typeof firstValue === "number" && typeof secondValue === "number") {
      ...
      comparison = String(firstValue).localeCompare(String(secondValue), "nl", {
        numeric: true,
        sensitivity: "base",
      });
    }
    ...
    return (
      ...
      >
        ...
        >
          <span>{label}</span>
          <SortIcon
            className={`h-3.5 w-3.5 shrink-0 transition-colors ${isActive ? "text-[#B8860B]" : "text-[#AAA295] group-hover:text-[#555555]"}`}
            aria-hidden="true"
          />
        </button>
      </th>
  ...
  const groupCounts = COLLECTION_GROUPS.reduce(
    (counts, group) => ({
      ...counts,
      [group.slug]: items.filter(
        (item) => getCollectionGroupForItem(item) === group.slug,
      ).length,
    }),
    {},
  );
  const editingCollectionGroup = getCollectionGroupForItem(
    editingItem || { itemType: "book" },
  );
  const editingTypeOptions = getItemTypesForGroup(editingCollectionGroup);
  const editingTypeDefinition = getItemTypeDefinition(
    editingItem?.itemType || "book",
  );
  ...
  return (
    <div className="admin-items space-y-6 text-[#111111] animate-fade-in">
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-3xl border border-[#D8CEB8] shadow-sm space-y-3">
          ...
          <p className="text-sm font-serif font-bold text-[#111111]">
            Geen items gevonden in collectie
          </p>
          <p className="text-xs text-[#888888]">
            Er zijn geen objecten die voldoen aan het huidige zoek- of
            filter-criteria.
          </p>
          <button
            onClick={() => {
              ...
              <tbody className="divide-y divide-[#EAE4D8]">
                {sortedItems.map((item) => {
                  const img =
                    item.images?.[0]?.url ||
                    item.image ||
                    "/images/scarron-spines-white-bg.jpg";
                    ...
                    >
                      ...
                      <td className="p-3.5 whitespace-nowrap">
                        <div
                          ...
                          data-admin-tooltip={getTranslationTooltip(
                            translationStatus,
                          )}
                        >
                          <div
                            className={`px-2.5 py-1 rounded-full text-[10px] font-mono font-bold flex items-center space-x-1 border cursor-help transition-all ${
                          ...
                          >
                            {translationStatus.isComplete ? (
                              ...
                              <>
                                ...
                                <span>
                                  {translationStatus.completeCount}/3 Talen
                                </span>
                              </>
                            )}
                          </div>
                      ...
                      <td className="p-3.5 whitespace-nowrap">
                        <span
                          className={`px-3 py-1 rounded-full text-[11px] font-serif font-bold ${
                      ...
                      <td className="p-3.5 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end space-x-1.5">
                          <button
                            type="button"
                            onClick={() =>
                              onOpenCertificate && onOpenCertificate(item)
                            }
                          ...
                          <button
                            ...
                            aria-label={
                              item.featured
                                ? "Topstukmarkering verwijderen"
                                : "Als topstuk markeren"
                            }
                            data-admin-tooltip={
                              item.featured
                                ? "Topstukmarkering verwijderen"
                          ...
                          >
                            <Star
                              className={`w-3.5 h-3.5 ${item.featured ? "fill-current" : ""}`}
                            />
                          </button>
                        </div>
                      </td>
          ...
          {filtered.map((item) => {
            ...
            const mainImgUrl =
              item.images && item.images[0]?.url
                ? item.images[0].url
                : item.image || "/images/scarron-spines-white-bg.jpg";
              ...
              >
                ...
                <div className="relative aspect-[4/3] w-full bg-[#FAF7F2] overflow-hidden border-b border-[#EAE4D8]">
                  ...
                  <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between z-10 pointer-events-auto">
                    ...
                    <div className="flex items-center space-x-1.5 flex-wrap gap-y-1">
                      ...
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-stone-100/90 backdrop-blur-md text-stone-800 border border-stone-300/80 flex items-center space-x-1 shadow-xs">
                        {React.createElement(getItemTypeIcon(item.itemType), {
                          className: "w-3 h-3 text-stone-600",
                        })}
                        <span>
                          {getLocalizedItemType(item.itemType, "nl", true)}
                        </span>
                      </span>
                  ...
                  <div className="absolute bottom-3.5 left-3.5 right-3.5 flex items-center justify-between z-10 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-1 group-hover:translate-y-0">
                    ...
                    >
                      <Star
                        className={`w-3.5 h-3.5 ${item.featured ? "fill-white" : ""}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                <div className="p-5 flex-grow flex flex-col justify-between space-y-3.5">
                  <div className="space-y-1.5">
                    <span className="text-[9.5px] font-mono font-bold text-[#B8860B] uppercase tracking-[0.15em] block truncate">
                      {getLocalizedCategoryLabel(item.category, "nl")} •{" "}
                      {item.century}
                    </span>
                ...
                <div className="px-5 py-3 bg-[#FAF7F2] border-t border-[#EAE4D8] flex items-center justify-between gap-2">
                  ...
                  >
                    <div
                      className={`px-2.5 py-1 rounded-full text-[10px] font-mono font-bold flex items-center space-x-1 border cursor-help transition-all duration-300 ${
                        translationStatus.isComplete
                          ? "bg-emerald-50 text-emerald-800 border-emerald-300 hover:bg-emerald-100"
                          : translationStatus.completeCount === 2
                            ? "bg-amber-50 text-amber-800 border-amber-300 hover:bg-amber-100"
      ...
      {editingItem &&
        createPortal(
          <div
            className="admin-item-editor fixed inset-0 z-[9999] w-screen h-screen bg-[#FAF8F5] text-[#1C1A18] flex flex-col font-sans overflow-hidden animate-fade-in"
            role="dialog"
            aria-modal="true"
            aria-label={
              isNew ? "Nieuw object invoeren" : `${editingItem.title} bewerken`
            }
          >
            ...
            <header className="admin-item-editor__header px-5 sm:px-7 py-3.5 bg-[#161412] text-white border-b border-[#2C2926] flex items-center justify-between gap-4 shrink-0 shadow-md">
              ...
              <div className="hidden lg:flex items-center space-x-1.5 bg-[#23201D] p-1.5 rounded-xl border border-[#332F2B]">
                {[
                  ...
                  const status =
                    getItemTranslationStatus(editingItem).details[lang.id];
              ...
              <div className="flex items-center space-x-2.5 shrink-0">
                ...
                >
                  ...
                  <span className="hidden xl:inline">
                    1. Kopieer AI Prompt ({formLang.toUpperCase()})
                  </span>
              ...
              <div className="admin-item-editor__sidebar lg:col-span-4 border-r border-[#E6E1D7] bg-[#F4F1EA] p-5 sm:p-6 space-y-6 overflow-y-auto">
                    ...
                    {COLLECTION_GROUPS.map((group) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            const allowedTypes = getItemTypesForGroup(
                              group.slug,
                            );
                            const currentType = editingItem.itemType || "book";
                            const nextType = allowedTypes.some(
                              (itemType) => itemType.slug === currentType,
                            )
                              ? currentType
                              : allowedTypes[0]?.slug || currentType;
                            const currentCategory = getCategorySlug(
                              editingItem.category,
                            );
                            const categoryIsValid = getCategoriesForGroup(
                              group.slug,
                            ).some(
                ...
                <div className="p-4 rounded-2xl bg-white border border-[#E0D9CC] space-y-3 shadow-xs">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {editingTypeOptions.map((itemType) => {
                      ...
                      const isActive =
                        (editingItem.itemType || "book") === itemType.slug;
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            setEditingItem({
                              ...
                              category: getCategoriesForGroup(
                        ...
                        >
                          ...
                          <span>
                            {getLocalizedItemType(itemType.slug, "nl")}
                          </span>
                        </button>
                      );
                    })}
                ...
                <div className="p-5 rounded-2xl bg-white border border-[#E0D9CC] space-y-4 shadow-xs">
                  ...
                  <div className="space-y-3.5">
                    ...
                    <label
                      className={`cursor-pointer flex items-center space-x-2.5 text-xs font-semibold p-3 rounded-xl border transition-all ${
                ...
                <div className="p-5 rounded-2xl bg-white border border-[#E0D9CC] space-y-4 shadow-xs">
                  <div className="flex items-center justify-between border-b border-[#E0D9CC] pb-2">
                    <h4 className="text-xs font-mono font-bold text-[#1C1A18] uppercase tracking-wider flex items-center space-x-2">
                      ...
                      <span>
                        Fotogalerij ({editingItem.images?.length || 0})
                      </span>
                    </h4>
                    <div className="flex items-center space-x-1.5">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setMediaPickerTarget({ type: "gallery" })
                        }
                        className="px-2 py-1 rounded-lg bg-[#F9F7F2] hover:bg-stone-200 text-[#1C1A18] text-[10px] font-mono font-bold border border-[#E0D9CC] transition-all"
                      >
                        + Beeldbank
                      </button>
                    </div>
                  ...
                  {showUrlInput && (
                    <form
                      ...
                      className="p-3 bg-[#F9F7F2] rounded-xl border border-[#E0D9CC] space-y-2 text-xs animate-fade-in"
                    >
                      <input
                        type="url"
                        value={newImageUrl}
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {editingItem.images?.map((img, idx) => (
                      <div
                        key={idx}
                        className="relative rounded-xl overflow-hidden border border-[#E0D9CC] group bg-[#F9F7F2] flex flex-col shadow-xs"
                      >
                        ...
                        <div className="p-1.5 bg-white border-t border-[#E0D9CC]">
                          <input
                            ...
                            value={img.caption || ""}
                            onChange={(e) =>
                              updateImageCaption(idx, e.target.value)
                            }
                ...
                <div className="admin-item-editor__tabs flex items-center justify-between border-b border-[#E0D9CC] pb-3 shrink-0">
                  <div className="flex items-center space-x-2">
                    {[
                      ...
                      {
                        id: "multilingual",
                        label: "2. Drietalige Inhoud & Staat",
                      },
                    ...
                    <div className="p-6 rounded-2xl bg-white border border-[#E0D9CC] space-y-5 shadow-xs">
                      <div>
                        ...
                        <input
                          ...
                          placeholder={
                            isFieldNvt("title", formLang)
                              ? "✓ Bewust leeg gelaten (N.v.t.)"
                              : editingFieldLabels.title
                          }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={
                            isFieldNvt("subtitle", formLang)
                              ? "✓ Bewust leeg gelaten (N.v.t.)"
                              : "Bijv. Parijs 1829 • 52 Delen Compleet"
                          }
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full px-3.5 py-2.5 rounded-xl bg-[#F9F7F2] border border-[#E0D9CC] text-sm text-[#1C1A18] font-semibold focus:outline-none focus:border-[#1C1A18]"
                          >
                            {getCategoriesForGroup(editingCollectionGroup).map(
                              (category) => (
                                <option
                                  key={category.slug}
                                  value={category.slug}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder={
                              isFieldNvt("city", formLang)
                                ? "✓ Bewust leeg gelaten (N.v.t.)"
                                : editingFieldLabels.city
                            }
                            className={`w-full px-3.5 py-2.5 rounded-xl border text-sm text-[#1C1A18] font-semibold focus:outline-none focus:border-[#1C1A18] ${
                              isFieldNvt("city", formLang)
                                ? "bg-amber-50/60 border-amber-300"
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="text-xs font-mono font-bold text-white uppercase tracking-wider">
                            Invoertaal voor onderstaande velden:{" "}
                          ...
                          <p className="text-[11px] text-stone-400 font-serif">
                            Geselecteerde inhoud wordt gekoppeld aan de{" "}
                            {formLang.toUpperCase()} taalweergave.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              updateFormField("binding", e.target.value)
                            }
                            placeholder={
                              isFieldNvt("binding", formLang)
                                ? "✓ Bewust leeg gelaten (N.v.t.)"
                                : editingFieldLabels.binding
                            }
                        ...
                        <div>
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              updateFormField("condition", e.target.value)
                            }
                            placeholder={
                              isFieldNvt("condition", formLang)
                                ? "✓ Bewust leeg gelaten (N.v.t.)"
                                : "Beschrijf de actuele staat en eventuele restauraties"
                            }
                            className={`w-full px-3.5 py-2.5 rounded-xl border text-xs text-[#1C1A18] focus:outline-none focus:border-[#1C1A18] leading-relaxed ${
                              isFieldNvt("condition", formLang)
                                ? "bg-amber-50/60 border-amber-300"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                          ...
                          <input
                            ...
                            placeholder={
                              isFieldNvt("dimensions", formLang)
                                ? "✓ Bewust leeg gelaten (N.v.t.)"
                                : "Bijv. In-8° (21,5 x 13,5 cm)"
                            }
                        ...
                        <div>
                          {renderFieldHeader(
                            editingFieldLabels.collationSpecs,
                            "collationSpecs",
                          )}
                          <input
                            type="text"
                            ...
                            onChange={(e) =>
                              updateFormField("collationSpecs", e.target.value)
                            }
                            placeholder={
                              isFieldNvt("collationSpecs", formLang)
                                ? "✓ Bewust leeg gelaten (N.v.t.)"
                                : "52 delen compleet. In-8°."
                            }
                    ...
                    <div className="p-6 rounded-2xl bg-white border border-[#E0D9CC] space-y-5 shadow-xs">
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          placeholder={
                            isFieldNvt("description", formLang)
                              ? "✓ Bewust leeg gelaten (N.v.t.)"
                              : "Schrijf hier het overzicht achter dit meesterwerk..."
                          }
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateFormField("historicalContext", e.target.value)
                          }
                          placeholder={
                            isFieldNvt("historicalContext", formLang)
                              ? "✓ Bewust leeg gelaten (N.v.t.)"
                              : "Schrijf hier de uitgebreide historische context..."
                          }
                          className={`w-full px-3.5 py-2.5 rounded-xl border text-xs text-[#1C1A18] focus:outline-none focus:border-[#1C1A18] leading-relaxed font-serif ${
                            isFieldNvt("historicalContext", formLang)
                              ? "bg-amber-50/60 border-amber-300"
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateFormField("provenance", e.target.value)
                          }
                          placeholder={
                            isFieldNvt("provenance", formLang)
                              ? "✓ Bewust leeg gelaten (N.v.t.)"
                              : "Ex-Libris Vacheron-Poinsot..."
                          }
                      ...
                      <div>
                        {renderFieldHeader(
                          editingFieldLabels.conditionReport,
                          "conditionReport",
                        )}
                        <textarea
                          rows={3}
                          ...
                          onChange={(e) =>
                            updateFormField("conditionReport", e.target.value)
                          }
                          placeholder={
                            isFieldNvt("conditionReport", formLang)
                              ? "✓ Bewust leeg gelaten (N.v.t.)"
                              : "Gedetailleerd conditierapport..."
                          }
                          className={`w-full px-3.5 py-2.5 rounded-xl border text-xs text-[#1C1A18] focus:outline-none focus:border-[#1C1A18] leading-relaxed font-serif ${
                            isFieldNvt("conditionReport", formLang)
                              ? "bg-amber-50/60 border-amber-300"
                      ...
                      <div>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateFormField("provenanceDetails", e.target.value)
                          }
                          placeholder={
                            isFieldNvt("provenanceDetails", formLang)
                              ? "✓ Bewust leeg gelaten (N.v.t.)"
                              : "Afkomstig uit het kasteelarchief..."
                          }
                          className={`w-full px-3.5 py-2.5 rounded-xl border text-xs text-[#1C1A18] focus:outline-none focus:border-[#1C1A18] leading-relaxed font-serif ${
                            isFieldNvt("provenanceDetails", formLang)
                              ? "bg-amber-50/60 border-amber-300"
                  ...
                  <div className="space-y-6 animate-fade-in">
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-stone-400 font-serif mt-1">
                            Voeg vergelijkbare verkopen toe. Een afbeelding is
                            het enige verplichte veld; zet “Tonen op website”
                            aan om de referentie publiek te tonen.
                          </p>
                        </div>
                      </div>
                      <button
                    ...
                    {(editingItem.comparableSales || []).length === 0 && (
                      <div className="py-14 px-6 text-center rounded-2xl border border-dashed border-[#CFC6B6] bg-white">
                        ...
                        <h4 className="text-base font-serif font-bold text-[#1C1A18]">
                          Nog geen vergelijkbare verkopen
                        </h4>
                        <p className="text-xs font-serif text-[#666666] mt-1">
                          Dit onderdeel blijft onzichtbaar op de productpagina.
                        </p>
                      </div>
                    )}
                    ...
                    {(editingItem.comparableSales || []).map((sale, index) => {
                      const descriptionField =
                        formLang === "nl"
                          ? "description"
                          : `description_${formLang}`;
                      const isComplete = isComparableSaleComplete(sale);

                      return (
                        <section
                          key={sale.id}
                          className="rounded-2xl bg-white border border-[#E0D9CC] shadow-xs overflow-hidden"
                        >
                          <header className="px-5 py-4 bg-[#F4F1EA] border-b border-[#E0D9CC] flex flex-wrap items-center justify-between gap-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <span
                                  className={`text-[10px] font-mono font-bold uppercase ${isComplete ? "text-emerald-700" : "text-amber-700"}`}
                                >
                                  {isComplete
                                    ? "Klaar om te tonen"
                                    : "Concept — afbeelding vereist"}
                                </span>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              <label
                                className={`min-h-9 px-3 rounded-lg border flex items-center gap-2 text-[11px] font-mono font-bold cursor-pointer ${
                                  sale.published
                                    ? "bg-emerald-50 border-emerald-300 text-emerald-800"
                                    : "bg-white border-[#D8CEB8] text-[#666666]"
                                }`}
                              >
                                <input
                                  type="checkbox"
                              ...
                              <button
                                ...
                                disabled={
                                  index ===
                                  editingItem.comparableSales.length - 1
                                }
                                title="Naar beneden"
                                className="w-9 h-9 rounded-lg border border-[#D8CEB8] bg-white hover:border-[#C5A059] disabled:opacity-30 cursor-pointer"
                              >
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2">
                                ...
                                <label className="min-h-10 px-3.5 py-2.5 rounded-xl bg-[#1C1A18] hover:bg-[#C5A059] text-white hover:text-black text-xs font-mono font-bold flex items-center justify-center gap-2 cursor-pointer transition-colors">
                                  ...
                                  <input
                                    ...
                                    onChange={(event) =>
                                      handleComparableImageUpload(index, event)
                                    }
                                  />
                                </label>
                                <button
                                  ...
                                  className="min-h-10 px-3.5 py-2.5 rounded-xl bg-[#F9F7F2] hover:bg-stone-200 text-[#1C1A18] text-xs font-mono font-bold border border-[#E0D9CC]"
                                >
                                  Beeldbank
                                </button>
                              </div>
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                  ...
                                  >
                                    ...
                                    <option value="including-premium">
                                      Inclusief opgeld
                                    </option>
                                  </select>
                                </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      if (itemToDelete.isBatch) {
                        ...
                        try {
                          await Promise.all(
                            selectedItemIds.map((id) => onDeleteItem(id)),
                          );
                          if (onShowToast)
                            onShowToast(
                              `${count} objecten verwijderd uit de collectie.`,
                            );
                          setSelectedItemIds([]);
                        } catch (error) {
                          if (onShowToast)
                            onShowToast(
                              "Niet alle objecten konden worden verwijderd.",
                              "error",
                            );
                        }
                      } else {
                        ...
                        if (onShowToast)
                          onShowToast(`"${itemToDelete.title}" verwijderd.`);
                      }
