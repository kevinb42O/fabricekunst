...
import { SUPPORTED_LANGUAGES, getLanguageAlternates, localizePath } from './locales.js';
...
function itemEntry(item, language) {
  ...
  const title = (language !== 'nl' && item[`title_${language}`]) || item.title || item.title_en || item.title_fr || SITE_NAME;
...
export function buildSitemapXml(catalogItems = []) {
  ...
  const staticEntries = STATIC_ROUTES.flatMap((route) => SUPPORTED_LANGUAGES.map((language) => {
    ...
    const image = route === '/'
      ...
      : '';
    return `  <url>\n    <loc>${SITE_URL}${localizedRoute}</loc>${alternateLinks(route)}${image}\n  </url>`;
  }));
  const itemEntries = uniqueItems.flatMap((item) => SUPPORTED_LANGUAGES.map((language) => itemEntry(item, language))).filter(Boolean);
