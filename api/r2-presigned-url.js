...
const PURPOSE_PREFIXES = {
  ...
  "rembrandt-project-hero": "rembrandt-project/hero",
  "rembrandt-project-update": "rembrandt-project/updates",
};
...
export default async function handler(req, res) {
  ...
  if (req.body?.action === "complete") {
    ...
    if (
      ...
      !["catalog/", "provenance/", "site/", "rembrandt-project/"].some(
  ...
  try {
    ...
    if (!Number.isSafeInteger(size) || size <= 0 || size > 20 * 1024 * 1024) {
      return res
        .status(413)
        .json({ error: "Image must be between 1 byte and 20 MB" });
    }
    const prefix = PURPOSE_PREFIXES[purpose];
    if (!prefix)
      return res.status(400).json({ error: "Unknown image upload purpose" });
    ...
    const uploadReceipt = createUploadReceipt(
      { objectKey, contentType, size, userId: authorization.user.id },
      process.env.R2_SECRET_ACCESS_KEY,
    );
